...
\href{https://www.repubblica.it/tecnologia/2023/07/06/news/innovation_4_change__industry_leaders_summit_cern_di_ginevra-406876410/}{Selection as one of the two CERN PhDs for the Collège des Ingeniéurs and CERN Ideasquare Innovation 4 Change (I4C) Program} \textbf{(\textit{1st Febrary 2023--4 July 2023})}\\
...
\textsc{OSX} -- \textsc{Windows} -- \textsc{bash} -- \textsc{C/C++} -- \textsc{Python3/2} -- \textsc{ROOT} -- \textsc{Numpy} -- \textsc{Pandas} -- \textsc{Scikit-learn} -- \textsc{Tensorflow} -- \textsc{Keras} -- \textsc{Matplotlib} -- \textsc{Spark Hadoop} -- \textsc{Git} -- \textsc{Office} -- \LaTeX -- \textsc{MATLAB} -- \textsc{HTML} -- \textsc{CSS} -- \textsc{Google Colab}
